...
describe('Task 9 - Comment System: Property-Based Tests', () => {
  ...
  describe('Property 18: Comments displayed in chronological order (Requirements 5.4)', () => {
    it('should display comments in chronological order (oldest first)', async () => {
      ...
      const existingPost = await prisma.post.findUnique({ where: { id: testPost.id } })
      const existingUser1 = await prisma.user.findUnique({ where: { id: testUser1.id } })
      const existingUser2 = await prisma.user.findUnique({ where: { id: testUser2.id } })
